...
<table cellpadding="5" style="margin-left: 30px" id="itemParamAddTable" class="itemParam">
	<tr>
		...
		<td><a href="javascript:void(0)" class="easyui-linkbutton selectItemCat">选择类目</a> 
			<input type="hidden" name="cid" style="width: 280px;" value=""></input>
		</td>
	</tr>
...
TAOTAO.initItemCat({
	...
	$(".addGroupTr").hide().find(".param").remove();
		//  判断选择的类目是否已经添加过规格
	  $.getJSON("/item/param/query/itemcatid/" + node.id+".do",function(data){
...
$("#itemParamAddTable .submit").click(function(){
	...
	console.log($("#itemParamAddTable [name=cid]").val());
	var url = "/item/param/update/"+$("#itemParamAddTable [name=cid]").val()+".do";
	$.post(url,{"paramData":JSON.stringify(params),"id":$("#itemeEditForm [name=id]").val()},function(data){
		if(data.status == 200){
